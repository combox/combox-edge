...
What it does:
  ...
  issue-server <dns_name>:
    - generates server cert for the provided DNS name (servers/<dns_name>.crt/.key)
...
issue_server_cert() {
  ...
  safe_name="$(printf '%s' "$dns_name" | tr '/\\ :\t' '_____' )"
  key_out="${SERVERS_DIR}/${safe_name}.key"
  csr_out="${SERVERS_DIR}/${safe_name}.csr"
  crt_out="${SERVERS_DIR}/${safe_name}.crt"
  tmp_ext="${SERVERS_DIR}/${safe_name}.ext"

  issue_cert "${dns_name}" "$key_out" "$csr_out" "$crt_out" "$tmp_ext"
  rm -f "$tmp_ext"
}
...
main() {
  ...
  case "$cmd" in
    ...
    issue-server)
      ...
      issue_server_cert "$2"
      echo "Server cert issued in ${SERVERS_DIR} for DNS:${2}" >&2
      ;;
    ""|-h|--help|help)
      usage
      ;;
